...
class WatermarkPosition extends Component {
    ...
    domCoverChangeHandler = (coverStyle, value) => {
        ...
        if (coverStyle == "colorCover") {
                cover.style.background = value;
                if(value){
                    this.setState({coverBackground : value})
                }
           }
    ...
    coverVideoController = (type) =>{
       let coverVid =  document.getElementById("videoCover");
   ...
   checkVideoWatermark = () =>{
    ...
    let previewCoverW = 640;
    let previewCoverH = 360;
    ...
    let videoMove = this.objectMoment(videoMoveEle.style.transform);
        videoMove.xMove = videoMove.xMove;

    let videoPreviewW = parseFloat(videoResizeEle.style.width);
    let videoPreviewH = (videoPreviewW/videoAspectRatio);
   

   let finalVideoData = {
       fW : (videoPreviewW/previewCoverW)*videoW,
       fH : (videoPreviewH/previewCoverH)*videoH,
       fX : (videoMove.xMove/previewCoverW)*videoW,
       fY : (videoMove.yMove/previewCoverH)*videoH
   }

   console.log(finalVideoData);

    let url =  `ffmpeg -f lavfi -i color=c=${this.state.coverBackground}:s=${videoW}x${videoH} -i demo.mp4 -filter_complex "[1]scale=${finalVideoData.fW}:${finalVideoData.fH} [i];[0][i]overlay=x=${finalVideoData.fX}:y=${finalVideoData.fY}:shortest=1,format=yuv420p" -y output.mp4`
    console.log(url);
}
    ...
    render() {
        ...
        // main cover style as per image aspect ratio
        ...
        return (
            <div>
                ...
                Cover Rotate <select id="flipCover"
                    ...
                    Cover Color <select id="colorCover"
                        onChange={e => this.domCoverChangeHandler("colorCover", e.target.value)}>
